...
dg_read_field_def_dtvalue(<<0:1, X:7, Rest/binary>>, N,
    ...
    case Key of
	  ...
	  d_field_dtvalue_counter_value(Rest, 0, 0, F1, F2, F3,
					TrUserData);
      18 ->
	  d_field_dtvalue_set_value(Rest, 0, 0, F1, F2, F3,
				    TrUserData);
      26 ->
	  d_field_dtvalue_map_value(Rest, 0, 0, F1, F2, F3,
				    TrUserData);
      _ ->
	  case Key band 7 of
	    0 ->
		skip_varint_dtvalue(Rest, 0, 0, F1, F2, F3, TrUserData);
	    1 ->
		skip_64_dtvalue(Rest, 0, 0, F1, F2, F3, TrUserData);
	    2 ->
		skip_length_delimited_dtvalue(Rest, 0, 0, F1, F2, F3,
					      TrUserData);
	    5 -> skip_32_dtvalue(Rest, 0, 0, F1, F2, F3, TrUserData)
	  end
    end;
...
d_field_dtvalue_set_value(<<0:1, X:7, Rest/binary>>, N,
    ...
    dfp_read_field_def_dtvalue(Rest2, 0, 0, F1,
			       cons(NewFValue, F2, TrUserData), F3, TrUserData).
...
dfp_read_field_def_dtop(<<26, Rest/binary>>, Z1, Z2, F1,
    ...
    d_field_dtop_map_op(Rest, Z1, Z2, F1, F2, F3,
			TrUserData);
dfp_read_field_def_dtop(<<>>, 0, 0, F1, F2, F3, _) ->
...
dg_read_field_def_dtop(<<0:1, X:7, Rest/binary>>, N,
    ...
    case Key of
      10 ->
	  d_field_dtop_counter_op(Rest, 0, 0, F1, F2, F3,
				  TrUserData);
      18 ->
	  d_field_dtop_set_op(Rest, 0, 0, F1, F2, F3, TrUserData);
      26 ->
	  d_field_dtop_map_op(Rest, 0, 0, F1, F2, F3, TrUserData);
      _ ->
	  case Key band 7 of
	    0 ->
		skip_varint_dtop(Rest, 0, 0, F1, F2, F3, TrUserData);
	    1 -> skip_64_dtop(Rest, 0, 0, F1, F2, F3, TrUserData);
	    2 ->
		skip_length_delimited_dtop(Rest, 0, 0, F1, F2, F3,
					   TrUserData);
	    5 -> skip_32_dtop(Rest, 0, 0, F1, F2, F3, TrUserData)
	  end
    end;
dg_read_field_def_dtop(<<>>, 0, 0, F1, F2, F3, _) ->
    #dtop{counter_op = F1, set_op = F2, map_op = F3}.
...
skip_varint_dtop(<<1:1, _:7, Rest/binary>>, Z1, Z2, F1,
		 F2, F3, TrUserData) ->
    skip_varint_dtop(Rest, Z1, Z2, F1, F2, F3, TrUserData);
...
d_field_dtupdateresp_map_value(<<0:1, X:7,
    ...
    NewFValue = id(d_msg_mapentry(Bs, TrUserData),
		   TrUserData),
    dfp_read_field_def_dtupdateresp(Rest2, 0, 0, F1, F2, F3,
				    F4, cons(NewFValue, F5, TrUserData),
				    TrUserData).
