...
-export([encode/1, encode_msg/1, encode_msg/2]).
-export([decode/2, decode_msg/2, decode_msg/3]).
...
-spec decode(atom(), binary()) -> no_return().
decode(MsgName, Bin) when is_atom(MsgName), is_binary(Bin) -> decode_msg(Bin, MsgName).
...
dfp_read_field_def_rpbbucketprops(<<8, Rest/binary>>,
    ...
    d_field_rpbbucketprops_n_val(Rest, Z1, Z2, F1, F2, F3,
				 ...
				 F23, F24, F25, F26, F27, F28, TrUserData);
...
dfp_read_field_def_rpbbucketprops(<<192, 1,
    ...
    d_field_rpbbucketprops_repl(Rest, Z1, Z2, F1, F2, F3,
				...
				F24, F25, F26, F27, F28, TrUserData);
...
d_field_rpbbucketprops_n_val(<<1:1, X:7, Rest/binary>>,
			     ...
			     F21, F22, F23, F24, F25, F26, F27, F28, TrUserData)
    when N < 57 ->
...
d_field_rpbbucketprops_repl(<<0:1, X:7, Rest/binary>>,
			    N, Acc, F1, F2, F3, F4, F5, F6, F7, F8, F9, F10,
			    F11, F12, F13, F14, F15, F16, F17, F18, F19, F20,
			    F21, F22, F23, _, F25, F26, F27, F28, TrUserData) ->
    <<Tmp:32/signed-native>> = <<(X bsl N +
				    Acc):32/unsigned-native>>,
...
skip_varint_rpbbucketprops(<<1:1, _:7, Rest/binary>>,
			   ...
			   F22, F23, F24, F25, F26, F27, F28, TrUserData) ->
...
merge_msg_tsinterpolation(#tsinterpolation{base =
					       PFbase,
					   interpolations = PFinterpolations},
			  #tsinterpolation{base = NFbase,
					   interpolations = NFinterpolations},
			  TrUserData) ->
    #tsinterpolation{base =
			 if NFbase =:= undefined -> PFbase;
			    true -> NFbase
			 end,
		     interpolations =
...
merge_msg_tscolumndescription(#tscolumndescription{name
						       = PFname,
						   type = PFtype},
			      #tscolumndescription{name = NFname,
						   type = NFtype},
			      _) ->
    #tscolumndescription{name =
...
merge_msg_tsrange(#tsrange{field_name = PFfield_name,
			   lower_bound = PFlower_bound,
			   lower_bound_inclusive = PFlower_bound_inclusive,
			   upper_bound = PFupper_bound,
			   upper_bound_inclusive = PFupper_bound_inclusive,
			   desc = PFdesc},
    ...
    #tsrange{field_name =
		 ...
		 if NFlower_bound_inclusive =:= undefined ->
			PFlower_bound_inclusive;
		    true -> NFlower_bound_inclusive
		 end,
	     upper_bound =
		 ...
		 if NFupper_bound_inclusive =:= undefined ->
			PFupper_bound_inclusive;
		    true -> NFupper_bound_inclusive
		 end,
	     desc =
...
merge_msg_tscoverageentry(#tscoverageentry{ip = PFip,
					   port = PFport,
...
merge_msg_tscoveragereq(#tscoveragereq{'query' =
					   PFquery,
				       table = PFtable,
...
merge_msg_tsdelreq(#tsdelreq{table = PFtable,
...
merge_msg_tsputreq(#tsputreq{table = PFtable,
			     columns = PFcolumns, rows = PFrows},
...
merge_msg_tsgetreq(#tsgetreq{table = PFtable,
			     key = PFkey, timeout = PFtimeout},
...
merge_msg_tslistkeysreq(#tslistkeysreq{table = PFtable,
				       timeout = PFtimeout},
...
get_msg_names() ->
    [rpbmodfun, rpbpair, tsinterpolation,
     tscolumndescription, tscell, rpbcommithook, tsrange,
     tscoverageentry, tsrow, rpbbucketprops,
     rpbresetbucketreq, rpbsetbuckettypereq,
     rpbgetbuckettypereq, tscoveragereq, tsdelreq, tsputreq,
     tsgetreq, rpbauthreq, tsqueryreq, tslistkeysreq,
     rpbsetbucketreq, rpbgetbucketreq, rpbgetserverinforesp,
     tscoverageresp, rpbgetbucketresp, tsdelresp, tsgetresp,
     tsqueryresp, rpberrorresp, tslistkeysresp, tsputresp].
